...
impl Connection {
	...
	pub fn New(WorkspaceId:String, DatabasePath:PathBuf) -> Self { Self { WorkspaceId, DatabasePath, Active:true } }
...
impl Manager {
	...
	pub fn New() -> Self {
		...
		Self { Connections:RwLock::new(HashMap::new()), DefaultPath, MaxCacheSize:10 }
	}
	...
	pub fn GetConnection(&self, WorkspaceId:&str) -> Result<Arc<RwLock<Connection>>, Kind> {
		...
		let Connection = Arc::new(RwLock::new(Connection::New(WorkspaceId.to_string(), DatabasePath)));
	...
	pub fn ConnectionCount(&self) -> usize {
		let Connections = self.Connections.read();
		Connections.values().filter(|c| c.read().Active).count()
	}
	...
	fn EnforceCacheSize(&self) {
		...
		while Connections.len() >= self.MaxCacheSize {
			...
			if let Some(InactiveKey) = Connections.iter().find(|(_, c)| !c.read().Active).map(|(k, _)| k.clone()) {
	...
	pub fn SetDefaultPath(&mut self, Path:PathBuf) { self.DefaultPath = Path; }

	/// Set maximum cache size
	pub fn SetMaxCacheSize(&mut self, Size:usize) { self.MaxCacheSize = Size; }
}
