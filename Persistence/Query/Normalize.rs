...
/// Prepare an FTS query by escaping special characters and handling column
/// prefixes
...
impl FtsQueryNormalizer {
	...
	pub fn prepare_fts_query(&self, query:&str) -> String { self.normalize_query(query) }
...
impl SearchQuery {
	...
	pub fn parse(query:&str) -> Self {
		...
		Self { terms, exclude_terms, phrase_searches }
	}
...
pub fn build_fts_where_clause(query:&str, columns:&[&str]) -> String {
	...
	let column_clauses:Vec<String> = columns.iter().map(|col| format!("{} MATCH ?", col)).collect();
