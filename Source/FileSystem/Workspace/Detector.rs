...
impl Detector {
	...
	/// Create a detector with default settings (current directory, max depth
	/// 10)
	pub fn NewDefault() -> Self { Self::New(None, 10) }
	...
	fn validate_workspace(&self, path:&Path, found_indicators:&[String]) -> bool {
		tracing::debug!(
			"Validating workspace at {} with indicators: {:?}",
			path.display(),
			found_indicators
		);
			...
			Ok(content) => {
				match serde_json::from_str::<serde_json::Value>(&content) {
					Ok(data) => {
						...
						let has_scripts = data
							.get("scripts")
							.and_then(|s| s.as_object())
							.map(|obj| {
								obj.contains_key("dev") || obj.contains_key("start") || obj.contains_key("build")
							})
							.unwrap_or(false);
						let has_dependencies =
							data.get("dependencies").is_some() || data.get("devDependencies").is_some();
						let is_module = data.get("type").and_then(|t| t.as_str()) == Some("module");

						let is_valid = has_name && (has_scripts || has_dependencies || is_module);
						tracing::debug!(
							"package.json validation: name={}, scripts={}, deps={}, module={}, valid={}",
							has_name,
							has_scripts,
							has_dependencies,
							is_module,
	...
	pub fn GetContextPortalPath(&self, workspace_root:&Path) -> PathBuf { workspace_root.join("context_portal") }
	...
	pub fn GetDetectionInfo(&self) -> DetectionInfo {
		...
		if info
			.indicators_found
			.iter()
			.any(|i| indicators::STRONG_INDICATORS.contains(&i.as_str()))
		{
		...
		info.environment_variables
			.insert("CONPORT_WORKSPACE".to_string(), std::env::var("CONPORT_WORKSPACE").ok());
		info.environment_variables.insert("PWD".to_string(), std::env::var("PWD").ok());
		if let Ok(cwd) = std::env::current_dir() {
			info.environment_variables
				.insert("CWD".to_string(), Some(cwd.to_string_lossy().to_string()));
		}
...
pub fn ResolveWorkspaceId(
	...
	tracing::warn!(
		"No workspace detection method available, using current directory: {}",
		fallback.display()
	);
	fallback
}
...
mod tests {
	...
	fn test_resolve_workspace_id_with_provided() {
		let result = ResolveWorkspaceId(Some("/custom/path".to_string()), true, None);
		assert_eq!(result, PathBuf::from("/custom/path"));
	}

	#[test]
	fn test_resolve_workspace_id_fallback() {
		let result = ResolveWorkspaceId(None, false, None);
