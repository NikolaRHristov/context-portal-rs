...
impl Default for MergeStrategy {
	fn default() -> Self { MergeStrategy::Merge }
}
...
impl<T> ParseResult<T> {
	pub fn new() -> Self { Self { items:vec![], errors:vec![], warnings:vec![] } }

	pub fn with_items(items:Vec<T>) -> Self { Self { items, errors:vec![], warnings:vec![] } }

	pub fn is_valid(&self) -> bool { self.errors.is_empty() }

	pub fn add_error(&mut self, error:String) { self.errors.push(error); }

	pub fn add_warning(&mut self, warning:String) { self.warnings.push(warning); }
}
...
pub fn ParseDecision(markdown:&str, _options:&ImportOptions) -> ParseResult<ParsedDecision> {
	...
	if result.is_valid() {
		result
			.items
			.push(ParsedDecision { summary, rationale, implementation_details, tags, timestamp });
	}
...
pub fn ParseDecisions(markdown:&str, options:&ImportOptions) -> ParseResult<ParsedDecision> {
	...
	let sections:Vec<&str> = markdown.split("\n---\n").filter(|s| s.contains("## Decision:")).collect();
...
pub fn ParseProgressList(markdown:&str, options:&ImportOptions) -> ParseResult<ParsedProgress> {
	...
	let sections:Vec<&str> = markdown
		.split("## ")
		.filter(|s| s.contains("TODO") || s.contains("In Progress") || s.contains("DONE"))
		.collect();
...
pub fn detect_data_type(markdown:&str) -> ImportDataType {
	if markdown.contains("# ConPort Contexts Export")
		|| markdown.contains("## Metadata") && markdown.contains("Workspace")
	{
	...
	} else if markdown.contains("# ConPort Progress Export")
		|| markdown.contains("TODO")
		|| markdown.contains("IN_PROGRESS")
	{
