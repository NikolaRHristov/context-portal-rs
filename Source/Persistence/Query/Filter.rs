...
// Handles building SQL for tag-based filtering with include_all and include_any
// support
...
impl TagFilterSql {
	pub fn new(where_clause:String, params:Vec<Box<dyn rusqlite::ToSql>>) -> Self { Self { where_clause, params } }

	pub fn is_empty(&self) -> bool { self.where_clause.is_empty() }
}
...
impl TagFilterInput {
	pub fn new_include_all(tags:Vec<String>) -> Self { Self { tags, mode:TagFilterMode::IncludeAll } }

	pub fn new_include_any(tags:Vec<String>) -> Self { Self { tags, mode:TagFilterMode::IncludeAny } }
...
/// * `include_all` - If true, items must have ALL tags (AND). If false, items
...
pub fn BuildTagFilterSql(tags:&[String], include_all:bool, tags_column:&str) -> Result<TagFilterSql, String> {
	if tags.is_empty() {
...
pub fn BuildTagFilterSqlJsonEach(tags:&[String], include_all:bool, tags_column:&str) -> Result<TagFilterSql, String> {
	if tags.is_empty() {
		return Ok(TagFilterSql::new(String::new(), vec![]));
	}
	...
	let subquery = if include_all {
		...
		let having_clause = format!("HAVING COUNT(DISTINCT value) = {}", tags.len());
...
pub fn validate_mutual_exclusivity(
	...
	let has_include_all = include_all.as_ref().map(|v| !v.is_empty()).unwrap_or(false);

	let has_include_any = include_any.as_ref().map(|v| !v.is_empty()).unwrap_or(false);

	if has_include_all && has_include_any {
		return Err(
			"Cannot use both tags_filter_include_all and tags_filter_include_any simultaneously. Use one or the \
			 other, not both."
				.to_string(),
		);
	}
...
impl FilterSqlBuilder {
	pub fn new(base_query:&str) -> Self { Self { base_query:base_query.to_string(), conditions:vec![], params:vec![] } }
	...
	pub fn with_tag_filter(mut self, tag_filter:&TagFilterInput, tags_column:&str) -> Result<Self, String> {
		let sql =
			BuildTagFilterSqlJsonEach(&tag_filter.tags, tag_filter.mode == TagFilterMode::IncludeAll, tags_column)?;
	...
	pub fn params(self) -> Vec<Box<dyn rusqlite::ToSql>> { self.params }
}
