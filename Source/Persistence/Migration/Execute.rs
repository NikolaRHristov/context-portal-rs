...
impl Execute {
	...
	fn MigrationV1(Connection:&Connection) -> Result<(), Kind> {
		...
		Connection
			.execute(
				"INSERT INTO SchemaVersion (Version, AppliedAt, Description) VALUES (1, datetime('now'), 'Initial \
				 schema')",
				[],
			)
			.map_err(|e| Kind::Database(e.to_string()))?;
	...
	pub fn GetSchemaVersion(Connection:&Connection) -> Result<i32, Kind> {
		let Result =
			Connection.query_row("SELECT MAX(Version) FROM SchemaVersion", [], |row| row.get::<_, Option<i32>>(0));
...
impl CreateTables {
	pub fn Execute(Connection:&Connection) -> Result<(), Kind> { Execute::CreateTables(Connection) }
}
...
impl RunMigrations {
	pub fn Execute(Connection:&Connection) -> Result<(), Kind> { Execute::RunMigrations(Connection) }
}
...
impl GetSchemaVersion {
	pub fn Execute(Connection:&Connection) -> Result<i32, Kind> { Execute::GetSchemaVersion(Connection) }
}
