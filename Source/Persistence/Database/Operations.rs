...
use crate::Type::{
	ContextLink::{ContextLink, GetLinkedItemsArgs, LinkConportItemsArgs},
	CustomData::{CustomData, GetCustomDataArgs, LogCustomDataArgs},
	Decision::{Decision, GetDecisionsArgs, LogDecisionArgs, UpdateDecisionArgs},
	History::{ContextHistory, GetItemHistoryArgs},
	Progress::{GetProgressArgs, LogProgressArgs, Progress, UpdateProgressArgs},
	SystemPattern::{GetSystemPatternsArgs, LogSystemPatternArgs, SystemPattern},
};
...
pub fn log_progress(conn:&Connection, args:&LogProgressArgs) -> Result<Progress, String> {
...
pub fn get_progress(conn:&Connection, workspace_id:&str, args:&GetProgressArgs) -> Result<Vec<Progress>, String> {
...
pub fn update_progress(conn:&Connection, workspace_id:&str, args:&UpdateProgressArgs) -> Result<bool, String> {
...
pub fn delete_progress(conn:&Connection, workspace_id:&str, progress_id:i64) -> Result<bool, String> {
	let rows = conn
		.execute(
...
pub fn log_system_pattern(conn:&Connection, args:&LogSystemPatternArgs) -> Result<SystemPattern, String> {
	let timestamp = current_timestamp();
	let tags_json = args
		.tags
		.as_ref()
	...
	conn.execute(
		"INSERT INTO SystemPatterns (WorkspaceId, Timestamp, Name, Description, Tags, CreatedAt)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
		params![args.workspace_id, timestamp, args.name, args.description, tags_json, timestamp,],
	)
	.map_err(|e| e.to_string())?;
...
pub fn get_system_patterns(
	...
	if let Some(ref tags) = args.tags_filter_include_all {
		...
		sql.push_str(&format!(
			" AND (Tags LIKE {} OR Tags IS NULL)",
			placeholders.join(" OR Tags LIKE ")
		));
		for tag in tags {
			params_vec.push(Box::new(format!("%\"{}%", tag)));
		}
...
pub fn delete_system_pattern(conn:&Connection, workspace_id:&str, pattern_id:i64) -> Result<bool, String> {
	let rows = conn
		.execute(
...
/// Log custom data
pub fn log_custom_data(conn:&Connection, args:&LogCustomDataArgs) -> Result<CustomData, String> {
	let timestamp = current_timestamp();
	...
	conn.execute(
		"INSERT INTO CustomData (WorkspaceId, Timestamp, Category, Key, Value, CreatedAt)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
		params![args.workspace_id, timestamp, args.category, args.key, value_json, timestamp,],
	)
	.map_err(|e| e.to_string())?;
...
/// Delete custom data
pub fn delete_custom_data(conn:&Connection, workspace_id:&str, category:&str, key:&str) -> Result<bool, String> {
	let rows = conn
		.execute(
...
pub fn search_custom_data_fts(
	...
	if let Some(cat) = category_filter {
		let rows = stmt
			.query_map(params![workspace_id, cat, query, limit], |row| {
				let value_str:String = row.get(5)?;
				let value:Value = serde_json::from_str(&value_str).unwrap_or(Value::Null);
		...
		let rows = stmt
			.query_map(params![workspace_id, query, limit], |row| {
				let value_str:String = row.get(5)?;
				let value:Value = serde_json::from_str(&value_str).unwrap_or(Value::Null);
...
pub fn add_context_history(
	...
	let version:i64 = conn
		.query_row(
			&format!(
				"SELECT COALESCE(MAX(Version), 0) + 1 FROM {} WHERE WorkspaceId = ?1",
				table_name
			),
			params![workspace_id],
			|row| row.get(0),
...
impl Context {
	...
	pub fn new(id:i64, content:serde_json::Value) -> Self { Self { Id:id, Content:content } }
}
...
pub fn get_product_context(conn:&Connection) -> Result<Context, String> {
	...
	let content:serde_json::Value =
		serde_json::from_str(&content_str).unwrap_or(serde_json::Value::Object(Default::default()));

	Ok(Context::new(1, content))
}

/// Update product context - supports full content or patch_content
pub fn update_product_context(conn:&Connection, workspace_id:&str, args:&UpdateContextArgs) -> Result<Context, String> {
	// Get current content
	let current_content_str:String = conn
		.query_row("SELECT Content FROM ProductContext WHERE Id = 1", [], |row| row.get(0))
		.map_err(|e| e.to_string())?;

	let mut current_content:serde_json::Value =
		serde_json::from_str(&current_content_str).unwrap_or(serde_json::Value::Object(Default::default()));
...
pub fn update_active_context(conn:&Connection, workspace_id:&str, args:&UpdateContextArgs) -> Result<Context, String> {
	// Get current content
	let current_content_str:String = conn
		.query_row("SELECT Content FROM ActiveContext WHERE Id = 1", [], |row| row.get(0))
		.map_err(|e| e.to_string())?;
...
pub fn create_link(conn:&Connection, args:&LinkConportItemsArgs) -> Result<ContextLink, String> {
	let timestamp = current_timestamp();

	conn.execute(
		"INSERT INTO ContextLinks (WorkspaceId, Timestamp, SourceItemType, SourceItemId, TargetItemType, \
		 TargetItemId, RelationshipType, Description)
...
pub fn get_linked_items(
	...
	let mut sql = String::from(
		"SELECT Id, WorkspaceId, Timestamp, SourceItemType, SourceItemId, TargetItemType, TargetItemId, \
...
/// Delete a link by ID
pub fn delete_link(conn:&Connection, workspace_id:&str, link_id:i64) -> Result<bool, String> {
	let rows = conn
		.execute(
...
/// Log a new decision
pub fn log_decision(conn:&Connection, args:&LogDecisionArgs) -> Result<Decision, String> {
	let timestamp = current_timestamp();
	let tags_json = args
		.tags
		.as_ref()
...
pub fn get_decisions(conn:&Connection, workspace_id:&str, args:&GetDecisionsArgs) -> Result<Vec<Decision>, String> {
	let mut sql = String::from(
...
pub fn get_decision_by_id(conn:&Connection, workspace_id:&str, decision_id:i64) -> Result<Option<Decision>, String> {
	let mut stmt = conn
		.prepare(
			"SELECT Id, WorkspaceId, Timestamp, Summary, Rationale, ImplementationDetails, Tags, CreatedAt
	...
	let mut rows = stmt
		.query_map(params![decision_id, workspace_id], |row| {
			Ok(Decision {
				Id:row.get(0)?,
				WorkspaceId:row.get(1)?,
				Timestamp:row.get(2)?,
...
/// Update a decision
pub fn update_decision(conn:&Connection, workspace_id:&str, args:&UpdateDecisionArgs) -> Result<bool, String> {
	let mut updates = Vec::new();
	let mut params_vec:Vec<Box<dyn rusqlite::ToSql>> = vec![Box::new(workspace_id.to_string())];
	...
	let sql = format!("UPDATE decisions SET {} WHERE Id = ? AND WorkspaceId = ?", updates.join(", "));
...
pub fn delete_decision(conn:&Connection, workspace_id:&str, decision_id:i64) -> Result<bool, String> {
	let rows = conn
		.execute(
...
pub fn search_decisions_fts(
	...
	workspace_id:&str,
	query_term:&str,
	_tags_filter:Option<&str>,
	limit:i64,
) -> Result<Vec<Decision>, String> {
	let query = format!("\"{}\"", query_term.replace("\"", "\"\""));

	let sql = "SELECT d.Id, d.WorkspaceId, d.Timestamp, d.Summary, d.Rationale, d.ImplementationDetails, d.Tags, \
	           d.CreatedAt
               FROM decisions d
...
pub fn get_recent_activity_summary(
	...
	let product_context:Vec<RecentContextHistory> = stmt
		.query_map(params![workspace_id, timestamp_str, limit_per_type], |row| {
			let content_str:String = row.get(4)?;
			let content:Value = serde_json::from_str(&content_str).unwrap_or(Value::Null);
	...
	let active_context:Vec<RecentContextHistory> = stmt
		.query_map(params![workspace_id, timestamp_str, limit_per_type], |row| {
			let content_str:String = row.get(4)?;
			let content:Value = serde_json::from_str(&content_str).unwrap_or(Value::Null);
			let id:i64 = row.get(0)?;
	...
	let progress_entries:Vec<RecentProgress> = stmt
		.query_map(params![workspace_id, timestamp_str, limit_per_type], |row| {
			let id:i64 = row.get(0)?;
	...
	let decisions:Vec<RecentDecision> = stmt
		.query_map(params![workspace_id, timestamp_str, limit_per_type], |row| {
			let id:i64 = row.get(0)?;
			Ok(RecentDecision {
				Id:id.to_string(),
	...
	let system_patterns:Vec<RecentSystemPattern> = stmt
		.query_map(params![workspace_id, timestamp_str, limit_per_type], |row| {
			let id:i64 = row.get(0)?;
			let tags_str:Option<String> = row.get(5)?;
			let tags:Option<Vec<String>> = tags_str.and_then(|s| serde_json::from_str(&s).ok());
	...
	let custom_data:Vec<RecentCustomData> = stmt
		.query_map(params![workspace_id, timestamp_str, limit_per_type], |row| {
			let id:i64 = row.get(0)?;
			let value_str:String = row.get(5)?;
			let value:Value = serde_json::from_str(&value_str).unwrap_or(Value::Null);
	...
	let context_links:Vec<RecentContextLink> = stmt
		.query_map(params![workspace_id, timestamp_str, limit_per_type], |row| {
			let id:i64 = row.get(0)?;
...
pub fn prepare_fts_query(query:&str, allowed_columns:Option<&[&str]>, default_column:Option<&str>) -> String {
	let q = query.trim();
	if q.is_empty() {
		return q.to_string();
	}
	...
	// If special characters that commonly break the parser are present, quote as
	// literal
	let special_chars = ['.', '/', '\\', '"'];
