...
use crate::{HTTP::Protocol::Response::Error, Persistence::Database::Connect::Connect};
...
pub async fn SearchDecisions(
	...
	Ok(Json(SearchResponse { Results:vec![], Total:0, Query:payload.Query }))
}
...
pub async fn SemanticSearch(
	...
	// For now, return empty results
	Ok(Json(SearchResponse { Results:vec![], Total:0, Query:String::new() }))
}
...
impl HandleSearchDecisions {
	pub async fn Execute(
		...
		let _Embedding = crate::AI::Embedding::Generate::Generate::Execute(&Payload.Query)
			.await
			.map_err(|e:crate::Error::Kind::Kind| {
		...
		Ok(SearchResponse { Results, Total:0, Query:Payload.Query })
	}
}
...
impl HandleSemanticSearch {
	pub async fn Execute(
		...
		Ok(SearchResponse { Results:results, Total:total, Query:Payload.Query })
	}
	...
	async fn GenerateEmbedding(
		...
		let embedding = if let Some(Model) = ModelGuard.as_ref() {
			Model.Generate(Content).map_err(|e| {
				Error::WithMessage(crate::HTTP::Protocol::Response::ErrorCode::InternalError, &e.to_string())
			})?
		} else {
		...
		match ItemType {
			"decision" => {
				let mut stmt = match conn
					.prepare("SELECT Summary, Rationale, Tags FROM Decisions WHERE Id = ? AND WorkspaceId = ?")
				{
					Ok(s) => s,
					Err(_) => return None,
				};

				let result:Result<(String, String, String), _> =
					stmt.query_row(params![ItemId, WorkspaceId], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)));
			...
			"progress_entry" => {
				...
				let result:Result<(String, String, Option<String>), _> =
					stmt.query_row(params![ItemId, WorkspaceId], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)));
			...
			"system_pattern" => {
				let mut stmt = match conn
					.prepare("SELECT Name, Description, Tags FROM SystemPatterns WHERE Id = ? AND WorkspaceId = ?")
				{
					Ok(s) => s,
					Err(_) => return None,
				};
			...
			"custom_data" => {
				let mut stmt = match conn
					.prepare("SELECT Category, Key, Value FROM CustomData WHERE Id = ? AND WorkspaceId = ?")
				{
					Ok(s) => s,
					Err(_) => return None,
				};
