...
pub async fn List(
	...
	let data_list =
		DbOps::get_custom_data(&conn, &args.workspace_id, &args).map_err(|e| crate::Error::Kind::Kind::Database(e))?;
...
pub async fn Create(
	...
	let data = DbOps::log_custom_data(&conn, &args).map_err(|e| crate::Error::Kind::Kind::Database(e))?;
...
pub async fn Search(
	...
	let safe_query =
		DbOps::prepare_fts_query(&args.query_term, Some(&["category", "key", "value_text"]), Some("value_text"));
