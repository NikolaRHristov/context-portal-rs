...
// Provides Initialize() function with tracing setup, FileAppender with daily
// rotation
...
pub fn Initialize() -> Result<(), Box<dyn std::error::Error>> {
	...
	let file_appender = RollingFileAppender::new(Rotation::DAILY, &log_dir, "conport.log");
...
pub fn InitializeWithWorkspace(workspace_id:&str) -> Result<(), Box<dyn std::error::Error>> {
	...
	tracing_subscriber::registry()
		.with(env_filter)
		.with(fmt::layer().with_writer(std::io::stdout).with_ansi(true).with_target(true))
		.with(fmt::layer().with_writer(file_appender).with_ansi(false).with_target(true))
		.init();

	tracing::info!(
		"Logging initialized for workspace {}. Log directory: {:?}",
		workspace_id,
		log_dir
	);
...
fn get_log_directory() -> Result<PathBuf, Box<dyn std::error::Error>> {
	let base_dir = dirs::data_local_dir().ok_or("Failed to get local data directory")?;
...
pub fn get_current_log_level() -> Level { parse_log_level(&get_log_level_from_env()) }
