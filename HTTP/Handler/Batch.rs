...
use crate::Persistence::Database::Operations;
use rusqlite::params;
...
impl HandleBatchLogItems {
    ...
    async fn ProcessDecision(
        ...
        let Rationale = Data.get("rationale")
            ...
            .unwrap_or("")
            .to_string();
        let ImplementationDetails = Data.get("implementation_details")
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .to_string();
        let Id = Data.get("id")
        ...
        let conn = DbState.Connection.lock().map_err(|e| crate::Error::Kind::Kind::DatabaseError(e.to_string()))?;
        let timestamp = crate::Persistence::Database::Connect::current_timestamp();
        let tags_json = Data.get("tags")
            .and_then(|v| serde_json::to_string(v).ok())
            .unwrap_or_default();
        
        conn.execute(
            "INSERT OR REPLACE INTO Decisions (Id, WorkspaceId, Summary, Rationale, ImplementationDetails, Tags, CreatedAt) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
    ...
    async fn ProcessProgressEntry(
        ...
        let Status = Data.get("status")
            ...
            .unwrap_or("TODO")
            .to_string();
        let ParentId = Data.get("parent_id")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        let Id = Data.get("id")
            .and_then(|v| v.as_str())
        ...
        conn.execute(
            "INSERT INTO ProgressEntries (WorkspaceId, Timestamp, Status, Description, ParentId, CreatedAt, UpdatedAt) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
    ...
    async fn ProcessSystemPattern(
        ...
        conn.execute(
            "INSERT OR REPLACE INTO SystemPatterns (WorkspaceId, Timestamp, Name, Description, Tags, CreatedAt) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
    ...
    async fn ProcessCustomData(
        ...
        conn.execute(
            "INSERT OR REPLACE INTO CustomData (WorkspaceId, Timestamp, Category, Key, Value, CreatedAt) VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
    ...
    /// HandleBatchLogItemsToDatabase - Log items directly to database without vector store
    ...
    impl HandleBatchLogItemsToDatabase {
        ...
        pub fn Execute(
            ...
            let conn = DbState.Connection.lock()
        ...
        fn StoreDecision(
            ...
            conn_guard.execute(
                "INSERT OR REPLACE INTO Decisions (Id, WorkspaceId, Summary, Rationale, ImplementationDetails, Tags, CreatedAt) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
                params![id, WorkspaceId, summary, rationale, implementation_details, tags_json, timestamp],
