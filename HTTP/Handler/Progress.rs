...
use crate::Type::Progress::{
    Progress, LogProgressArgs, GetProgressArgs, UpdateProgressArgs, DeleteProgressByIdArgs,
    ProgressResponse,
};
...
pub async fn List(
    ...
    let progress_entries = DbOps::get_progress(&conn, &args.workspace_id, &args)
        .map_err(|e| crate::Error::Kind::Kind::Database(e))?;

    Ok(Json(progress_entries.into_iter().map(|p| ProgressResponse {
        id: p.Id,
...
pub async fn Get(
    ...
    let progress_entries = DbOps::get_progress(&conn, &args.workspace_id, &GetProgressArgs {
    ...
    if let Some(p) = progress_entries.into_iter().find(|p| p.Id.parse::<i64>().map(|i| i == id).unwrap_or(false)) {
...
pub async fn Create(
    ...
    let progress = DbOps::log_progress(&conn, &args)
        .map_err(|e| crate::Error::Kind::Kind::Database(e))?;
...
pub async fn Update(
    ...
    let updated = DbOps::update_progress(&conn, &args.workspace_id, &args)
        .map_err(|e| crate::Error::Kind::Kind::Database(e))?;
