...
use crate::Type::SystemPattern::{
    SystemPattern, LogSystemPatternArgs, GetSystemPatternsArgs, DeleteSystemPatternByIdArgs,
...
pub async fn List(
    ...
    Ok(Json(patterns.into_iter().map(|p| SystemPatternResponse {
        id: p.Id,
...
pub async fn Create(
    ...
    let pattern = DbOps::log_system_pattern(&conn, &args)
        .map_err(|e| crate::Error::Kind::Kind::Database(e))?;
