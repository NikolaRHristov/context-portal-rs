...
impl HandleExportMarkdown {
    pub async fn Execute(
        ...
        let OutputPath = Payload.OutputPath.unwrap_or_else(|| format!("./conport_export_{}", Payload.WorkspaceId));
        ...
        for ItemType in &Payload.ItemTypes {
            let (Items, Filename) = Self::ExportItemType(
                DbState,
                VectorStore,
                &Payload.WorkspaceId,
                ItemType,
...
impl HandleImportMarkdown {
    pub async fn Execute(
        ...
        let MarkdownFiles = Self::FindMarkdownFiles(InputPath)
            .map_err(|e| Error::WithMessage(
                crate::HTTP::Protocol::Response::ErrorCode::InternalError,
                &e.to_string(),
            ))?;
