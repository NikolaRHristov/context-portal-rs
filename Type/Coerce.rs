...
// Provides CoerceStringToInt for digit-only strings and CoerceOptional for Option fields
...
impl CoerceStringToInt for i32 {
    ...
    fn coerce_from_optional(s: Option<&str>) -> Option<Self> {
        s.and_then(|v| Self::coerce_from_string(v))
    }
}
...
pub fn is_digit_only(s: &str) -> bool {
    !s.is_empty() && s.chars().all(|c| c.is_ascii_digit())
}

/// Coerce a string to an integer, returning None if not digit-only
pub fn coerce_string_to_int<T: CoerceStringToInt>(s: &str) -> Option<T> {
    T::coerce_from_string(s)
}
...
impl<T: CoerceStringToInt> CoerceOptional<T> for T {
    fn coerce(opt: Option<String>) -> Option<T> {
        opt.and_then(|v| T::coerce_from_string(v.as_str()))
    }
}
...
pub fn coerce_to_i32(s: &str) -> Option<i32> {
    coerce_string_to_int(s)
}

/// Coerce a string field to i64
pub fn coerce_to_i64(s: &str) -> Option<i64> {
    coerce_string_to_int(s)
}

/// Coerce a string field to u32
pub fn coerce_to_u32(s: &str) -> Option<u32> {
    coerce_string_to_int(s)
}

/// Coerce a string field to u64
pub fn coerce_to_u64(s: &str) -> Option<u64> {
    coerce_string_to_int(s)
}

/// Coerce an optional string to i32
pub fn coerce_optional_to_i32(s: Option<String>) -> Option<i32> {
    coerce_optional_string_to_int(s)
}

/// Coerce an optional string to i64
pub fn coerce_optional_to_i64(s: Option<String>) -> Option<i64> {
    coerce_optional_string_to_int(s)
}

/// Coerce an optional string to u32
pub fn coerce_optional_to_u32(s: Option<String>) -> Option<u32> {
    coerce_optional_string_to_int(s)
}

/// Coerce an optional string to u64
pub fn coerce_optional_to_u64(s: Option<String>) -> Option<u64> {
    coerce_optional_string_to_int(s)
}
...
pub fn coerce_optional_to_bool(s: Option<String>) -> Option<bool> {
    s.and_then(|v| coerce_to_bool(&v))
}

/// Coerce a string, trimming whitespace
pub fn coerce_trim(s: &str) -> String {
    s.trim().to_string()
}
