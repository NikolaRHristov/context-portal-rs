...
impl SystemPattern {
	pub fn New(WorkspaceId:String, Name:String, Description:Option<String>, Tags:Option<Vec<String>>) -> Self {
	...
	pub fn with_id(
		...
		let now = chrono::Utc::now().to_rfc3339();
		Self { Id, WorkspaceId, Timestamp:now.clone(), Name, Description, Tags, CreatedAt:now }
	}

	/// Serialize tags to JSON string for database storage
	pub fn tags_to_json(&self) -> Option<String> {
		self.Tags.as_ref().map(|tags| serde_json::to_string(tags).unwrap_or_default())
	}
...
impl GetSystemPatternsArgs {
	...
	pub fn validate(&self) -> Result<(), String> {
		...
		if self.tags_filter_include_all.is_some() && self.tags_filter_include_any.is_some() {
			return Err(
				"Cannot use 'tags_filter_include_all' and 'tags_filter_include_any' simultaneously".to_string(),
			);
		}
...
mod tests {
	...
	fn test_log_system_pattern_args_validation() {
		...
		let invalid_args =
			LogSystemPatternArgs { workspace_id:"ws1".to_string(), name:"".to_string(), description:None, tags:None };
