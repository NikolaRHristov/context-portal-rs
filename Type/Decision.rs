...
impl From<Decision> for DecisionResponse {
    fn from(d: Decision) -> Self {
        let tags: Option<Vec<String>> = d.Tags.as_ref()
...
impl SearchDecisionsArgs {
    pub fn validate(&self) -> Result<(), String> {
        let has_include_all = self.tags_filter_include_all
            .as_ref()
            .map(|v| !v.is_empty())
            .unwrap_or(false);
        let has_include_any = self.tags_filter_include_any
            .as_ref()
            .map(|v| !v.is_empty())
            .unwrap_or(false);
...
impl Decision {
    pub fn New(workspace_id: String, summary: String, rationale: Option<String>, implementation_details: Option<String>, tags: Option<Vec<String>>) -> Self {
        let timestamp = chrono::Utc::now().to_rfc3339();
        let tags_json = tags.as_ref()
